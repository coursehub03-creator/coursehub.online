...
function guardInstructor() {
  ...
  document.getElementById("instructorName") &&
    (document.getElementById("instructorName").textContent = user.name || "الأستاذ");
  return user;
}
...
async function loadMetrics(uid) {
  ...
  const enrollments = enrollmentsSnap.docs
    .map((d) => d.data())
    .filter((x) => courseIds.has(x.courseId));
  const certs = certsSnap.docs
    .map((d) => d.data())
    .filter((x) => courseIds.has(x.courseId));
...
function fillDashboard(data) {
  ...
  const inReview = data.submissions.filter((s) =>
    ["pending", "submitted", "under_review", "resubmitted"].includes(String(s.status || ""))
  ).length;

  const set = (id, value) =>
    document.getElementById(id) && (document.getElementById(id).textContent = String(value));
          ...
          .map(
            (s) => `
              <div style="padding:8px 0;border-bottom:1px solid var(--color-border)">
                <strong>${s.title || "دورة بدون عنوان"}</strong>
                ${badge(s.status || "submitted")}
                ${(s.reviewReason || s.note)
                  ? `<p>ملاحظات المراجعة: ${s.reviewReason || s.note}</p>`
                  : ""}
              </div>
            `
          )
...
function fillCourses(data) {
  ...
  data.enrollments.forEach((x) =>
    enrollByCourse.set(x.courseId, (enrollByCourse.get(x.courseId) || 0) + 1)
  );

  const doneByCourse = new Map();
  data.certs.forEach((x) =>
    doneByCourse.set(x.courseId, (doneByCourse.get(x.courseId) || 0) + 1)
  );
  ...
  rows.innerHTML = data.courses.length
    ? data.courses
        .map((c) => {
          ...
          return `
            <tr>
              ...
              <td>${r}%</td>
              <td>${note}</td>
              <td>${needsAction ? "<a class='ch-btn secondary' href='/instructor-builder.html'>تعديل وإعادة إرسال</a>" : "-"}</td>
            </tr>
          `;
        })
        .join("")
    : "<tr><td colspan='6'>لا توجد دورات بعد.</td></tr>";
}
...
function fillStudents(data) {
  ...
  data.courses.forEach((c) =>
    byCourse.set(c.id, { title: c.title || "-", enrolled: 0, completed: 0 })
  );

  data.enrollments.forEach((e) => {
    if (byCourse.get(e.courseId)) byCourse.get(e.courseId).enrolled += 1;
  });

  data.certs.forEach((c) => {
    if (byCourse.get(c.courseId)) byCourse.get(c.courseId).completed += 1;
  });

  rows.innerHTML = [...byCourse.values()].length
    ? [...byCourse.values()]
        .map(
          (x) => `
            <tr>
              ...
              <td>${x.enrolled}</td>
              <td>${x.completed}</td>
              <td>${x.enrolled ? Math.round((x.completed / x.enrolled) * 100) : 0}%</td>
            </tr>
          `
        )
        .join("")
        ...
        .map(([courseId, x]) => {
          ...
          return `
            <tr>
              <td>${title}</td>
              <td>${x.attempts}</td>
              <td>${Math.round(x.total / x.attempts)}</td>
            </tr>
          `;
        })
        .join("")
    : "<tr><td colspan='3'>لا توجد محاولات اختبارات بعد.</td></tr>";

  const totalAttempts = [...map.values()].reduce((a, b) => a + b.attempts, 0);
  const totalScore = [...map.values()].reduce((a, b) => a + b.total, 0);

  document.getElementById("reportTotalAttempts") &&
    (document.getElementById("reportTotalAttempts").textContent = totalAttempts);
  document.getElementById("reportAvgScore") &&
    (document.getElementById("reportAvgScore").textContent = totalAttempts ? Math.round(totalScore / totalAttempts) : 0);
  document.getElementById("reportTotalCourses") &&
    (document.getElementById("reportTotalCourses").textContent = data.courses.length);
}
