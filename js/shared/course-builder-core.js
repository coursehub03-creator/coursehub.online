...
const questionTemplate = () => ({ id: uid(), question: "", explanation: "", options: ["", "", "", ""], correctIndexes: [] });
const slideElement = (type = "text") => ({ id: uid(), type, x: 40, y: 40, w: type === "heading" ? 320 : 260, h: type === "shape" ? 120 : 80, z: 1, text: type === "heading" ? "عنوان" : "نص", src: "", style: { background: "#ffffff", color: "#0f172a", radius: 10 } });
const slideTemplate = (i = 1) => ({ id: uid(), title: `شريحة ${i}`, background: "#f8fafc", elements: [slideElement("heading")] });
const lessonTemplate = (i = 1) => ({
  ...
  checkpointQuiz: { title: "اختبار نقطة تحقق", passingScore: 70, timeLimitMinutes: 10, questions: [questionTemplate()] }
});
const moduleTemplate = (i = 1) => ({ id: uid(), title: `وحدة ${i}`, lessons: [lessonTemplate(1)] });
const courseTemplate = () => ({
  ...
  finalQuiz: { title: "الاختبار النهائي", passingScore: 70, timeLimitMinutes: 20, questions: [questionTemplate()] },
...
export function createCourseBuilder({ role = "instructor", selectors = {} }) {
  ...
  async function loadCategories() {
    ...
    q("#category").innerHTML = state.categories.map((n) => `<option value="${esc(n)}">${esc(n)}</option>`).join("");
  }

  function applyCourseToForm() {
    ["title", "subtitle", "slug", "category", "description", "outcomes", "requirements", "cover", "promoVideo", "level", "language"].forEach((k) => {
      const el = q(`#${k}`);
  ...
  function setStep(next) {
    ...
    q("#stepBar").innerHTML = STEPS.map((s, i) => `<button type="button" class="step-pill ${i === state.step ? "active" : ""}">${i + 1}. ${s}</button>`).join("");
  ...
  function bindGeneralInputs() {
    q("#builderForm").addEventListener("input", (e) => {
      ...
      if (target.matches("[data-bind]")) state.course[target.dataset.bind] = target.type === "number" ? Number(target.value || 0) : target.value;
      autosave();
      renderPreview();
    });
  ...
  function renderModules() {
    ...
    root.innerHTML = state.course.modules.map((mod) => `
      <article class="module-card">
        ...
        <div class="lesson-list">
          ${mod.lessons.map((l) => `<button type="button" class="lesson-chip ${l.id === state.activeLessonId ? "active" : ""}" data-open-lesson="${l.id}" data-module-id="${mod.id}"><span>${esc(l.title)}</span><span class="lesson-state">${statusLabel(l.status)}</span></button>`).join("")}
        </div>
      </article>
    `).join("");

    root.querySelectorAll("[data-module-title]").forEach((el) => el.addEventListener("input", () => {
      const mod = state.course.modules.find((m) => m.id === el.dataset.moduleTitle);
      if (mod) mod.title = el.value;
      autosave();
      renderPreview();
    }));
    root.querySelectorAll("[data-add-lesson]").forEach((btn) => btn.addEventListener("click", () => {
    ...
    root.querySelectorAll("[data-open-lesson]").forEach((btn) => btn.addEventListener("click", () => {
      state.activeModuleId = btn.dataset.moduleId;
      state.activeLessonId = btn.dataset.openLesson;
      renderAll();
    }));
    root.querySelectorAll("[data-del-module]").forEach((btn) => btn.addEventListener("click", () => {
  ...
  function renderLessonSettings() {
    ...
    root.innerHTML = `
      <div class="lesson-settings-grid">
        ...
        <label>الحالة<select id="lessonStatus" class="ch-select">${WORKFLOW.map((s) => `<option value="${s}" ${lesson.status === s ? "selected" : ""}>${statusLabel(s)}</option>`).join("")}</select></label>
        <label>نوع المحتوى<select id="lessonContentType" class="ch-select">${CONTENT_TYPES.map((t) => `<option value="${t}" ${lesson.contentType === t ? "selected" : ""}>${t}</option>`).join("")}</select></label>
      </div>
    ...
    q("#lessonTitle").oninput = (e) => { lesson.title = e.target.value; renderModules(); renderPreview(); autosave(); };
    q("#lessonDuration").oninput = (e) => { lesson.durationMinutes = Number(e.target.value || 1); renderPreview(); autosave(); };
    q("#lessonStatus").onchange = (e) => { lesson.status = e.target.value; renderModules(); autosave(); };
    q("#lessonContentType").onchange = (e) => { lesson.contentType = e.target.value; autosave(); };
    q("#lessonSummary").oninput = (e) => { lesson.summary = e.target.value; renderPreview(); autosave(); };
    q("#lessonContent").oninput = (e) => { lesson.content = e.target.value; autosave(); };
    q("#lessonPreviewToggle").onchange = (e) => { lesson.allowPreview = e.target.checked; renderPreview(); autosave(); };
  ...
  function renderSlides() {
    ...
    slideList.innerHTML = lesson.slides.map((s, i) => `<button class="slide-thumb ${s.id === state.activeSlideId ? "active" : ""}" type="button" data-open-slide="${s.id}">${i + 1}. ${esc(s.title)}</button>`).join("");
    const slide = activeSlide();
    q("#slideBackground").value = slide.background || "#f8fafc";
    canvas.style.background = slide.background || "#f8fafc";
    canvas.innerHTML = slide.elements.map((el) => {
    ...
    slideList.querySelectorAll("[data-open-slide]").forEach((btn) => btn.addEventListener("click", () => {
      ...
      <div class="quiz-card">
        ...
        ${quiz.questions.map((qz, i) => `
          <article class="quiz-question" data-quiz-key="${key}" data-q-id="${qz.id}">
            ...
            ${qz.options.map((opt, oi) => `<div class="quiz-option-row"><input class="ch-input" data-opt-index="${oi}" value="${esc(opt)}" placeholder="الخيار ${oi + 1}"><label><input type="checkbox" data-correct-index="${oi}" ${qz.correctIndexes.includes(oi) ? "checked" : ""}> صحيح</label></div>`).join("")}
            <div class="inline-actions">
  ...
  function renderQuizzes() {
    const lesson = activeLesson();
    q("#lessonQuizEditor").innerHTML = lesson ? quizEditor(lesson.checkpointQuiz, "lesson") : `<div class="empty-box">اختر درساً أولاً.</div>`;
    q("#finalQuizEditor").innerHTML = quizEditor(state.course.finalQuiz, "final");

    document.querySelectorAll("[data-quiz-meta]").forEach((el) => el.addEventListener("input", () => {
      const quiz = resolveQuiz(el.dataset.quizMeta);
      quiz[el.dataset.field] = el.type === "number" ? Number(el.value || 0) : el.value;
      autosave();
    }));
    document.querySelectorAll("[data-add-question]").forEach((btn) => btn.addEventListener("click", () => {
      resolveQuiz(btn.dataset.addQuestion).questions.push(questionTemplate());
      renderQuizzes();
      autosave();
    }));
    document.querySelectorAll("[data-add-option]").forEach((btn) => btn.addEventListener("click", () => {
    ...
    document.querySelectorAll("[data-del-question]").forEach((btn) => btn.addEventListener("click", () => {
    ...
    document.querySelectorAll(".quiz-question").forEach((row) => {
      ...
      if (!qz) return;
      row.querySelectorAll("[data-q-field]").forEach((el) => el.addEventListener("input", () => { qz[el.dataset.qField] = el.value; autosave(); }));
      row.querySelectorAll("[data-opt-index]").forEach((el) => el.addEventListener("input", () => { qz.options[Number(el.dataset.optIndex)] = el.value; autosave(); }));
      row.querySelectorAll("[data-correct-index]").forEach((el) => el.addEventListener("change", () => {
        const idx = Number(el.dataset.correctIndex);
        qz.correctIndexes = el.checked ? [...new Set([...(qz.correctIndexes || []), idx])] : (qz.correctIndexes || []).filter((i) => i !== idx);
        autosave();
      }));
    });
  }
  ...
  function validationErrors(mode = "submit") {
    ...
    q("#validationList").innerHTML = errs.length ? errs.map((x) => `<li>${esc(x)}</li>`).join("") : `<li>✅ جميع عناصر النشر مكتملة.</li>`;
    return errs;
  }

  function payload(nextStatus) {
    const lessons = state.course.modules.flatMap((m) => m.lessons.map((l) => ({ ...l, moduleId: m.id, moduleTitle: m.title })));
  ...
  async function publishDirect() {
    ...
    if (state.course.id) await setDoc(doc(db, "courses", state.course.id), data, { merge: true });
    else {
  ...
  async function restoreDraft() {
    ...
    if (role === "instructor") {
      const snap = await getDocs(query(collection(db, "instructorCourseSubmissions"), where("instructorId", "==", state.user.uid), orderBy("updatedAt", "desc"), limit(5)));
      const notes = snap.docs.map((d) => d.data()).map((x) => ({ status: x.status, note: x.note || x.reviewReason || "" })).filter((x) => x.note || x.status);
  ...
  function renderPreview() {
    ...
    preview.innerHTML = `
      ...
      <section class="preview-curriculum"><h4>منهج الدورة</h4><ul>${state.course.modules.map((m) => `<li><strong>${esc(m.title)}</strong> (${m.lessons.length} دروس)</li>`).join("")}</ul></section>
      <section class="preview-lesson"><h4>معاينة الدرس الحالي</h4><p>${esc(lesson?.title || "-")}</p><p>${esc(lesson?.summary || "")}</p>${slide ? `<div class="slide-preview-mini" style="background:${slide.background}">${slide.elements.map((el) => `<span style="left:${el.x / 4}px;top:${el.y / 4}px;width:${el.w / 4}px;height:${el.h / 4}px">${esc(el.text || el.type)}</span>`).join("")}</div>` : ""}</section>
      <section class="preview-quiz"><h4>${esc(state.course.finalQuiz.title || "الاختبار النهائي")}</h4><p>عدد الأسئلة: ${state.course.finalQuiz.questions.length} • النجاح: ${state.course.finalQuiz.passingScore}%</p></section>
    `;
  }
  ...
  function renderReviewNotes() {
    ...
    q("#reviewTimeline").innerHTML = notes.length
      ? notes.map((n) => `<article><strong>${statusLabel(n.status || "")}</strong><p>${esc(n.note || "")}</p></article>`).join("")
      : `<div class="empty-box">لا توجد ملاحظات مراجعة حالياً.</div>`;
    if (role !== "instructor") q("#instructorReviewPanel").innerHTML = `<div class="empty-box">هذه اللوحة مخصصة للأستاذ.</div>`;
  }
