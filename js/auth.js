...
const messages = {
  ar: {
    ...
    emailInUse: "هذا البريد مستخدم مسبقًا. سجّل الدخول أو استخدم نسيت كلمة المرور.",
    storageUnauthorized: "لا يمكن رفع شهادة العمل الآن بسبب صلاحيات التخزين. تواصل مع الإدارة لضبط Firebase Storage Rules.",
    registerInProgress: "جاري إنشاء الحساب..."
  },
  en: {
    ...
    emailInUse: "This email is already in use. Please log in or reset your password.",
    storageUnauthorized: "Work-proof upload is blocked by storage permissions. Ask admin to update Firebase Storage rules.",
  ...
  registerForm.addEventListener("submit", async (e) => {
    ...
    try {
      if (submitBtn) {
        ...
        submitBtn.textContent = t("registerInProgress");
      }
      ...
      if (createdUser && error?.code === "storage/unauthorized") {
        try {
          ...
          console.error("Failed to cleanup created user after storage error:", cleanupError);
        }
      }
